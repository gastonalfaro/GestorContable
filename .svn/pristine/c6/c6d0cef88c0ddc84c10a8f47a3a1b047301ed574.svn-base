...
{
    public partial class ImpresionFormulario : BASE
    {
        ...
        private void GenerarReporte(string str_IdRevelacion)
        {
            ...
            try
            {
                ...
                if ((!String.IsNullOrEmpty(strUsuario)) && !String.IsNullOrEmpty(Request.QueryString["Rev"]))
                {
                    ...
                    oParametros.Add(new ReportParameter("pIdRevelacion", str_IdRevelacion, false));
                    oParametros.Add(new ReportParameter("pResultado", "", false));
                    oParametros.Add(new ReportParameter("pMensaje", "", false));
                    oParam = new ParametrosReporte(ConfigurationManager.AppSettings["ReporteRevelacion"], oParametros, ConfigurationManager.AppSettings["ServidorReportes"]);
        ...
        {
            string lstr_Url = String.Format("~/RevelacionNotas/Consulta.aspx?Rev={0}", Request.QueryString["Rev"]);
