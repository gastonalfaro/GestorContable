...
{
    public class CancelacionesAnticipadas
    {
        /*
         Cancelar titulos valores antes de la fecha de vencimiento
         
         * el monto de los intereses ganados por el inversionista y el efecto de los montos devengados a la fecha de la cancelación anticipada
         *Tipos 
         * Cero Cupon (Con/Sin descuento)
         * Tasa Fija (Con/Sin descuento)
         * Tasa Variable (Con/Sin descuento)
         * 
         * 
         * Cargar los titulosValor (tomar la fecha de vencimiento)
         * Clasificar por el asiento (Por los tipos)
         * 
         * 
         */
        ...
        public string CancelacionAnticipada() 
        {
            ...
            try
            {
                ...
                //wsDeudaInterna.CalculaDevengoValores(); ojo cucurucho 20190603 función sin parametros
                wsDeudaInterna.CalculaDevengoValores(0, 0, "", "");
                
                DataTable ldat_Nemotecnicos = new DataTable();
                ldat_Nemotecnicos = ws_SGService.uwsConsultarNemotecnicos(null, null, null, null, null).Tables[0];
                ...
                for (int i = 0; i < ldat_TitulosValores.Rows.Count; i++)
                {
                    ...
                    //if (ldat_Nemotecnicos.Select("IdNemotecnico = '" + ldat_TitulosValores.Rows[i]["NemoTecnico"].ToString() + "'").Count() != 0)
                        ...
                        && lstr_moneda == (ldat_TitulosValores.Rows[i]["Moneda"].ToString().Equals("CRCN") ? "CRC" : ldat_TitulosValores.Rows[i]["Moneda"].ToString()))
                    {
                        ...
                        //    )
                        //{
                        if (ldat_TitulosValores.Rows[i]["EstadoValor"].ToString() == "Vigente")
                        {
                            string resultado = wsDeudaInterna.CrearTituloValor(
                                Convert.ToInt32(ldat_TitulosValores.Rows[i]["NroValor"].ToString()),
                                ...
                                ldat_TitulosValores.Rows[i]["TipoNegociacion"].ToString(),
                                ldat_TitulosValores.Rows[i]["Moneda"].ToString().Equals("CRC") ? "CRCN" : ldat_TitulosValores.Rows[i]["Moneda"].ToString(),
                                Convert.ToDecimal(ldat_TitulosValores.Rows[i]["ValorFacial"].ToString()),
                                ldat_TitulosValores.Rows[i]["FchValor"].ToString().Substring(0, 10),
                                ldat_TitulosValores.Rows[i]["PlazoValor"].ToString(),
                                ldat_TitulosValores.Rows[i]["FchCancelacion"].ToString().Substring(0, 10),
                                ldat_TitulosValores.Rows[i]["FchVencimiento"].ToString().Substring(0, 10),
                                ...
                                Convert.ToDecimal(ldat_TitulosValores.Rows[i]["TasaBruta"].ToString()),
                                Convert.ToDecimal(ldat_TitulosValores.Rows[i]["TasaNeta"].ToString()),
                                Convert.ToDecimal(ldat_TitulosValores.Rows[i]["Margen"].ToString()),
                                ldat_TitulosValores.Rows[i]["NroEmisionSerie"].ToString(),
                                ldat_TitulosValores.Rows[i]["Propietario"].ToString(),
                                ldat_TitulosValores.Rows[i]["EntidadCustodia"].ToString(),
                                ldat_TitulosValores.Rows[i]["FchCreacion"].ToString().Substring(0, 10),
                                ldat_TitulosValores.Rows[i]["SistemaNegociacion"].ToString(),
                                ldat_TitulosValores.Rows[i]["MotivoAnulacion"].ToString(),
                                ...
                                ldat_TitulosValores.Rows[i]["ModuloSINPE"].ToString(),
                                "SG",
                                "Descripción Negociación", //cucurucho nuevo
                                "Numero Identificacion", //cucurucho  nuevo
                                "TipoIdentificacion").ToString();//cucurucho  nuevo tambien se agrega .ToString
                    ...
                    {
                        mensaje = mensaje + ldat_TitulosValores.Rows[i]["NumeroValor"].ToString() + ", ";
                    }
                }
            }
            ...
            string lstr_Detalle)
        {
            ...
            if (lstr_EstadoValor == "Cancelado")
            {
                    ...
                    case "tasa variable":
                        {
                                ...
                                (ldt_FchValor.Year == ldt_FchVencimiento.Year))
                            {
                                ...
                                foreach (DataRow ldr_Row in ldat_Tira.Rows)
                                {
                                    ...
                                    if (ldat_Tira.Rows[0]["IdPosPre"].ToString().Trim().Equals("PP_Balance"))
                                    {
                                        //ldat_Reservas = ws_SGService.uwsConsultarReservaDetallado(string.Empty);  cucucrucho
                                        ldat_Reservas = ws_SGService.uwsConsultarReservaDetallado(string.Empty, "", "", "", "", "", "", "", "", "", "", "", "", "");
               ...
               lstr_Mensaje = GenerarAsientoAjuste(ldat_Asiento, lstr_Operacion, lstr_NroValor, lstr_Nemotecnico, ldec_TipoCambio);
            }
            return lstr_Mensaje;
        }
